...
void test_device_method_with_string(IOTHUB_CLIENT_TRANSPORT_PROVIDER protocol, const char *payload)
{
    ...
    conn_info.conn_status = IOTHUB_CLIENT_CONNECTION_UNAUTHENTICATED;
    // Initialize locking
    conn_info.lock = Lock_Init();
    ASSERT_IS_NOT_NULL(conn_info.lock);
    ...
    do
    {
        ...
        {
            if (conn_info.conn_status == IOTHUB_CLIENT_CONNECTION_AUTHENTICATED)
            {
